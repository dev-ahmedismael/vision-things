...
const Services = () => {
  ...
  useEffect(() => {
    axios.get(url).then((res) => {
      ...
      let foundPage = pages.find(
        (page) => `/en/services/${page.slug}` == pathname
      );
      setPage(foundPage);
    });
  }, []);
  return (
    <Box py={5}>
      {primaryContent && (
        <CenteredSection
          title={primaryContent.title_en}
          description={primaryContent.content_en}
        />
      )}
      {secondaryContent && (
        <section>
            ...
            <Box py={5}>
              <Grid container spacing={5}>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  >
                    {secondaryContent.title_en !== "na" && (
                      <Box width={"fit-content"}>
                        <Title>
                          <Typography color={"white"} variant="h4">
                            {secondaryContent.title_en}
                          </Typography>
                        </Title>
                      </Box>
                    )}
