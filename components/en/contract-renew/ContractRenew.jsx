  ...
  const search = (formData) => {
    axios.get(`${searchUrl}/${formData.contract_number}`).then((res) => {
      if (res.data.length === 0) {
        setErrMsg(
          "Contract not found, please check your number and try again."
        );
      } else {
        setErrMsg(null);
        setResponseMsg(
          `We've found a contract in the name of Mr. ${res.data[0].name} `
        );
      ...
      <Container>
        <Box py={5}>
          <Typography textAlign={"center"} variant="h4" mb={5}>
            Cameras Contract Renew
          </Typography>
          <Box id="contract-renew">
